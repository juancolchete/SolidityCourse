...
describe("WhiteList", () =>{
    ...
    it("Should print that you are in the whitelist", async ()=>{
        await whitelist.addToWhiteList("0x1D8989d956A015AE1333d9ee2a7a42b403426dFd")
        let printResponse = await whitelist.isInList("0x1D8989d956A015AE1333d9ee2a7a42b403426dFd");
        expect(printResponse).to.be.equal(true);
    });
    it("Should print that you are not in the whitelist", async ()=>{
        let printResponse = await whitelist.isInList("0xBd3D3C0bCc3FBdDedd019277012ab2da6BB294F5");
    ...
    it("Should remove the address from the whitelist and check if the address is really out", async ()=>{
        await whitelist.remove("0x1D8989d956A015AE1333d9ee2a7a42b403426dFd")
        let printResponse = await whitelist.isInList("0x1D8989d956A015AE1333d9ee2a7a42b403426dFd");
        expect(printResponse).to.be.equal(false);
    });
